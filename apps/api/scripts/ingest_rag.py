...
Uso:
    uv run python -m scripts.ingest_rag --institution <uuid> --title "Referencia PSeInt" archivo.md
"""
...
async def _ingest(institution_id: uuid.UUID, title: str, text: str) -> None:
    session_factory = get_session_factory()
    async with session_factory() as db:
        document = await ingest_document(db, institution_id=institution_id, title=title, text=text)
        await db.commit()
...
def main() -> None:
    ...
    asyncio.run(_ingest(uuid.UUID(args.institution), args.title, text))
